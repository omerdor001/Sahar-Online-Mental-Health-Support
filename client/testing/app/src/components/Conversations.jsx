...
import { fetchConversationSummary } from "../http.js";
...
function Conversations({ handleLogout }) {
  ...
  const [conversationsLastUpdateTime, setConversationsLastUpdateTime] =
    useState();
  ...
  const handleSummarizeConversation = async () => {
    ...
    try {
      ...
      setConversationSummary("שגיאה בעת בקשת הסיכום.");
    } finally {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("openConversationsUpdated", handleUpdate);
  }, []);

  useEffect(() => {
    let filtered = [];
    if (openConversations && Array.isArray(openConversations)) {
      filtered = openConversations.filter(
        (conversation) =>
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Box sx={{ p: 1, borderRadius: 2, backgroundColor: "#f1f8ff" }}>
              {" "}
              {/* Reduced padding */}
              <Stack
                direction="row"
              ...
              >
                {["אחוז גלובלי", "כינוי", "שם הסייע/ת", "מזהה שיחה"].map(
                  (header, idx) => (
                    <Box
                      key={idx}
                      sx={{
        ...
        <Modal open={modalOpen} onClose={handleCloseModal}>
          <Box
            sx={{
              ...
              "&::-webkit-resizer": {
                background:
                  "linear-gradient(-45deg, transparent 0%, transparent 35%, #1976d2 35%, #1976d2 45%, transparent 45%, transparent 65%, #1976d2 65%, #1976d2 75%, transparent 75%)",
                width: "16px",
                height: "16px",
              },
            }}
            ...
            <Box
              ...
              onMouseDown={(e) => {
                ...
                const handleMouseMove = (moveEvent) => {
                  ...
                  newLeft = Math.max(
                    10,
                    Math.min(viewportWidth - modalWidth - 10, newLeft)
                  );
                  newTop = Math.max(
                    10,
                    Math.min(viewportHeight - modalHeight - 10, newTop)
                  );
                      ...
                      <>
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            סיכום השיחה בתהליך, ויהיה מוכן בקרוב...
                          </Typography>
                        </Box>
                      </>
                    ) : (
            ...
            >
              ...
              <Box
                sx={{
                  ...
                  background:
                    "linear-gradient(-45deg, transparent 0%, transparent 35%, #999 35%, #999 45%, transparent 45%, transparent 65%, #999 65%, #999 75%, transparent 75%)",
                  cursor: "nw-resize",
                }}
              />
            </Box>
