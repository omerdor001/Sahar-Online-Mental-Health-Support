...
export async function fetchConversationSummary(conversationId) {
  ...
  const response = await fetch(`${API_BASE_URL}/summarize_conversation`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `${token}`,
  ...
  const resData = await response.json();
  return resData.message; // Your backend returns { message: "successfully started ..." }
}
